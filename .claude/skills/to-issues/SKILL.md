...
- **Screen-touching slices carry a visual contract.** If the slice includes a screen, its acceptance criteria MUST include a visual-conformance checkbox (e.g. "matches the linked design frame and the conventions in `docs/design.md`") — prose references are advisory; **only checkboxes bind**. Its References section MUST link the **specific design frame** for that screen (Figma or equivalent), not just the whole design file.
...
- For screen slices: the **specific design frame** for this screen + `docs/design.md`.
