...
**Status:** accepted · **Date:** 2026-06-30 · **Clarified:** 2026-07-02 (round half-up once at line total; reconciliation requires exactly-zero drift)
...
- Rounding is **half-up, applied once at the line total** (full precision on intermediates).
- Because rounding happens exactly once, the reconciliation invariant
  ([ADR-0010](0010-net-worth-definition.md)) requires **exactly-zero drift — no rounding
  tolerance.** Any non-zero drift is a bug, not noise.

## Open follow-ups
- None — round half-up once at the line total; reconciliation requires exactly-zero drift.
